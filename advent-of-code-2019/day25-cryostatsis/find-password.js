...
const getInput = (i, instructions, input) => {
  if (input.length === 0) {
    const inp = prompt("").trim() + "\n";
    input.push(...(inp.split("").map((x) => x.charCodeAt())));
  }
  instructions[i] = input.shift();
};
  ...
  while (i < instructions.length) {
    ...
    switch (opcode) {
      ...
      case 3:
        ...
        outputs = [];
        getInput(i1, instructions, input);
        i += 2;
        break;
      case 4:
        i1 = getAddress(mode1, instructions, 1, relativeBase, i);
        // printOutput(i1, instructions);
        outputs.push(String.fromCharCode(instructions[i1]));
        i += 2;
        break;
      case 5:
        i1 = getAddress(mode1, instructions, 1, relativeBase, i);
